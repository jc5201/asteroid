...
def dataset_generator_full(target_dir,
                      normal_dir_name="normal",
                      abnormal_dir_name="abnormal",
                      ext="wav"):
    """
    ...
    for dir in target_dir:
        normal_files.extend(sorted(glob.glob(
        os.path.abspath("{dir}/{normal_dir_name}/*.{ext}".format(dir=dir,
                                                                 normal_dir_name=normal_dir_name,
                                                                 ext=ext)))))
    ...
    for dir in target_dir:
        ...
        os.path.abspath("{dir}/{abnormal_dir_name}/*.{ext}".format(dir=dir,
                                                                   abnormal_dir_name=abnormal_dir_name,
                                                                   ext=ext)))))                              
    abnormal_labels = numpy.ones(len(abnormal_files))
    if len(abnormal_files) == 0:
...
if __name__ == "__main__":
    ...
    dirs = []
    for id in ids:
        dirs.extend(sorted(glob.glob(os.path.abspath("{base}/6dB/valve/{id}/".format(base=param["base_directory"], id = id)))))
    ...
    train_files, train_labels, eval_files, eval_labels = dataset_generator_full(dirs)
