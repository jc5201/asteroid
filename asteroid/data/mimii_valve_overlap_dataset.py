...
class MIMIIValveOverlapDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.data[index]:
            ...
            for i, source in enumerate(sources_tmp):
                ...
                if source == sources_tmp[0]:
                    audio1, label1 = self.select_audio_file(src_idx)
                    active_label_sources[source] = label1
                
                else:
                    audio2, label2 = self.select_audio_file(src_idx)
                                            
                    # calculate overlap ratio
                    overlap_rate = torch.sum(active_labels[0,0,:] * label2[0,:])
                    overlap_ratio = (overlap_rate/torch.sum(active_labels[0,0,:])) + (overlap_rate/torch.sum(label[0,:]))
                    overlap_ratio = overlap_ratio/2
                    
                    while overlap_ratio < self.overlap_ratio:
                        audio2, label2 = self.select_audio_file(src_idx)
                        
                        overlap_rate = torch.sum(active_labels[0,0,:] * label2[0,:])
                        overlap_ratio = (overlap_rate/torch.sum(active_labels[0,0,:])) + (overlap_rate/torch.sum(label2[0,:]))
                        overlap_ratio = overlap_ratio/2
                        
                    active_label_sources[source] = label2
            ...
            self.data[index]["audio_sources"] = audio_sources
            self.data[index]["active_labels"] = active_labels
        else:
            return self.data[index]["audio_mix"], self.data[index]["audio_sources"], self.data[index]["active_labels"]
    ...
    def select_audio_file(self, src_idx):
    ...
    def calculate_overlap_ratio(self, active_labels, label):
        overlap_rate = torch.sum(active_labels[0, 0, :] * label[0, :])
        overlap_ratio = (overlap_rate / torch.sum(active_labels[0, 0, :])) + (overlap_rate / torch.sum(label[0, :]))
        return overlap_ratio /2
