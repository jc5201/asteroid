...
def freq_domain_loss(s_hat, gt_spec, combination=True):
    ...
    for i in range(n_src):
        ...
        loss = F.leaky_relu(inferences[i] - refrences[i], negative_slope=0.5) ** 2
        _loss_mse += loss.mean()
...
class MultiDomainLoss(_Loss):
    ...
    def forward(self, est_targets, targets, return_est=False, **kwargs):
        ...
        # targets[:, 0, :, :] 에 mixture 넣어놨음. mixture_t 가져와야 해서.
        mixture_t = targets[:, 0, :, :]
        targets = targets[:, 1:, :, :]
        ...
        if self._multi:
            ...
            # mixture_t = sum([targets[:, i : i + 1, ...] for i in range(n_src)])
